...
#based on ICD Issue G draft
#use for data produced according to ICD G draft
#change name to read_paket.py (and replace read_paket.py in the directory, which is currently based on ICD Issue F)
#alternatively adjust import call in read_racfile.py 
...
def read_payload_data(packet_type,service_type,service_subtype,sid,cont_package,block_length,payload_data):
    ...
    elif packet_type == 'TM' and service_type == 1 and service_subtype == 2:
        data = read_payload_generic_data(payload_data,sid,block_length) #Generic data (not yet specified)
    elif packet_type == 'TM' and service_type == 1 and service_subtype == 7:
        data = read_payload_generic_data(payload_data,sid,block_length) #Generic data (not yet specified)
    elif packet_type == 'TM' and service_type == 1 and service_subtype == 8:
        data = read_payload_generic_data(payload_data,sid,block_length) #Generic data (not yet specified)
    else:
...
def read_payload_housekeeping_data(payload_data, sid, block_length_test):
    ...
    if sid == 1:
        ...
        data['SVNA'] = numpy.fromstring(binascii.unhexlify(payload_data[12:14]),numpy.dtype('<u1'))
        data['SVNB'] = numpy.fromstring(binascii.unhexlify(payload_data[14:16]),numpy.dtype('<u1'))
        data['SVNC'] = numpy.fromstring(binascii.unhexlify(payload_data[16:18]),numpy.dtype('<u1'))
        data['TS'] = numpy.fromstring(binascii.unhexlify(payload_data[18:26]),numpy.dtype('<u4'))
        data['TSS'] = numpy.fromstring(binascii.unhexlify(payload_data[26:30]),numpy.dtype('<u1'))
        data['MODE'] = numpy.fromstring(binascii.unhexlify(payload_data[30:32]),numpy.dtype('<u1'))
        if int(data['MODE'])<1 or int(data['MODE'])>2:
            print('WARNING: MODE='+str(data['MODE'])+' is out of range')
        data['EDACE'] = numpy.fromstring(binascii.unhexlify(payload_data[32:40]),numpy.dtype('<u4'))
        data['EDACCE'] = numpy.fromstring(binascii.unhexlify(payload_data[40:48]),numpy.dtype('<u4'))
        data['EDACN'] = numpy.fromstring(binascii.unhexlify(payload_data[48:56]),numpy.dtype('<u4'))
        data['SPWEOP'] = numpy.fromstring(binascii.unhexlify(payload_data[56:64]),numpy.dtype('<u4'))
        data['SPWEEP'] = numpy.fromstring(binascii.unhexlify(payload_data[64:72]),numpy.dtype('<u4'))
        data['ANOMALY'] = numpy.fromstring(binascii.unhexlify(payload_data[72:74]),numpy.dtype('<u1'))
        if int(data['ANOMALY'])<0 or int(data['ANOMALY'])>255:
            print('WARNING: ANOMALY='+str(data['ANOMALY'])+' is out of range')
        ...
        data['HTR7A'] = numpy.fromstring(binascii.unhexlify(payload_data[24:28]),numpy.dtype('<u2'))
        if int(data['HTR7A'])<0 or int(data['HTR7A'])>4095:
            print('WARNING: HTR7A='+str(data['HTR7A'])+' is out of range')
        data['HTR7B'] = numpy.fromstring(binascii.unhexlify(payload_data[28:32]),numpy.dtype('<u2'))
        if int(data['HTR7B'])<0 or int(data['HTR7B'])>4095:
            print('WARNING: HTR7B='+str(data['HTR7B'])+' is out of range')
        data['HTR7OD'] = numpy.fromstring(binascii.unhexlify(payload_data[32:36]),numpy.dtype('<u2'))
        if int(data['HTR7OD'])<0 or int(data['HTR7OD'])>4095:
            print('WARNING: HTR7OD='+str(data['HTR7OD'])+' is out of range')
        data['HTR8A'] = numpy.fromstring(binascii.unhexlify(payload_data[36:40]),numpy.dtype('<u2'))
        if int(data['HTR8A'])<0 or int(data['HTR8A'])>4095:
            print('WARNING: HTR7A='+str(data['HTR7A'])+' is out of range')
        data['HTR8B'] = numpy.fromstring(binascii.unhexlify(payload_data[40:44]),numpy.dtype('<u2'))
        if int(data['HTR8B'])<0 or int(data['HTR8B'])>4095:
            print('WARNING: HTR8B='+str(data['HTR8B'])+' is out of range')
        data['HTR8OD'] = numpy.fromstring(binascii.unhexlify(payload_data[44:48]),numpy.dtype('<u2'))
...
def read_payload_transparent_data(payload_data, sid, cont_packet, block_length_test):
    ...
    data = {}
    if sid in numpy.arange(21,28,1).tolist():
        ## CCD data
        ...
        if cont_packet == False:
            ...
            #beginning of CRB module produced data, coloured red in ICD
            data['FRAME'] = numpy.fromstring(binascii.unhexlify(payload_data[22:26]), numpy.dtype('<u2'))
            data['NROW'] = numpy.fromstring(binascii.unhexlify(payload_data[26:30]), numpy.dtype('<u2'))
            if int(data['NROW'])<1 or int(data['NROW'])>511:
                 print('WARNING: NROW='+str(data['NROW'])+' is out of range')
            data['NRBIN'] = numpy.fromstring(binascii.unhexlify(payload_data[30:34]), numpy.dtype('<u2'))
            if int(data['NRBIN'])<0 or int(data['NRBIN'])>63:
                 print('WARNING: NRBIN='+str(data['NRBIN'])+' is out of range')
            data['NRSKIP'] = numpy.fromstring(binascii.unhexlify(payload_data[34:38]), numpy.dtype('<u2'))
            if int(data['NRSKIP'])<0 or int(data['NRSKIP'])>512:
                 print('WARNING: NRSKIP='+str(data['NRSKIP'])+' is out of range')
            data['NCOL'] = numpy.fromstring(binascii.unhexlify(payload_data[38:42]), numpy.dtype('<u2'))
            if int(data['NCOL'])<0 or int(data['NCOL'])>2047:
                 print('WARNING: NCOL='+str(data['NCOL'])+' is out of range')
            data['NCBIN'] = numpy.fromstring(binascii.unhexlify(payload_data[42:46]), numpy.dtype('<u2'))
            data['NCSKIP'] = numpy.fromstring(binascii.unhexlify(payload_data[46:50]), numpy.dtype('<u2'))
            if int(data['NCSKIP'])<0 or int(data['NCSKIP'])>2047:
                 print('WARNING: NCSKIP='+str(data['NCSKIP'])+' is out of range')
            data['NFLUSH'] = numpy.fromstring(binascii.unhexlify(payload_data[50:54]), numpy.dtype('<u2'))
            if int(data['NFLUSH'])<0 or int(data['NFLUSH'])>1023:
                 print('WARNING: NFLUSH='+str(data['NFLUSH'])+' is out of range')
            data['TEXPMS'] = numpy.fromstring(binascii.unhexlify(payload_data[54:62]), numpy.dtype('<u4'))
            data['GAIN'] = numpy.fromstring(binascii.unhexlify(payload_data[62:66]), numpy.dtype('<u2'))
            data['TEMP'] = numpy.fromstring(binascii.unhexlify(payload_data[66:70]), numpy.dtype('<u2'))
            data['FBINOV'] = numpy.fromstring(binascii.unhexlify(payload_data[70:74]), numpy.dtype('<u2'))
            if int(data['FBINOV'])<0 or int(data['FBINOV'])>32767:
                 print('WARNING: FBINOV='+str(data['FBINOV'])+' is out of range')
            data['LBLNK'] = numpy.fromstring(binascii.unhexlify(payload_data[74:78]), numpy.dtype('<u2'))
            data['TBLNK'] = numpy.fromstring(binascii.unhexlify(payload_data[78:82]), numpy.dtype('<u2'))
            data['ZERO'] = numpy.fromstring(binascii.unhexlify(payload_data[82:86]), numpy.dtype('<u2'))
            data['TIMING1'] = numpy.fromstring(binascii.unhexlify(payload_data[86:90]), numpy.dtype('<u2'))
            data['TIMING2'] = numpy.fromstring(binascii.unhexlify(payload_data[90:94]), numpy.dtype('<u2'))
            data['VERSION'] = numpy.fromstring(binascii.unhexlify(payload_data[94:98]), numpy.dtype('<u2'))
            data['TIMING3'] = numpy.fromstring(binascii.unhexlify(payload_data[98:102]), numpy.dtype('<u2'))
            data['NBC'] = numpy.fromstring(binascii.unhexlify(payload_data[102:106]), numpy.dtype('<u2'))
            data['BC'] = numpy.fromstring(binascii.unhexlify(payload_data[106:106+int(data['NBC'])*2]),numpy.dtype('<u2'))
            #end of CRB module produced data
            data['IMG'] = payload_data[106+int(data['NBC'])*2:]        
